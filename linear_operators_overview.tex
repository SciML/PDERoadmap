...
\fi
Similarly, as $P$ is defined by \eqref{P_definition} and the interiors are solved by \eqref{solve_u_hat_cond_1} and \eqref{solve_u_hat_cond_2}:
\begin{align}
u = (P Q_L)^{-1}(x-P Q_b)
\end{align}
...
\end{align}
\fi
Given $P$ defined by \eqref{P_definition}, the rest steps for solving interiors, $u$, and the extended state vector $\bar{u}$, are similar with what we did for previous examples.
