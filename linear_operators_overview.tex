...
\section{More Details}
		...
		\begin{itemize}
		\item Define $S_E$ as the set of non interior grid point indexes, e.g, if we have a univariate problem with just three non non interior point, let's say, the first one and the last two grid points, then $M_E = 3$ and $S_E = \{1,\bar{M}-1,\bar{M}\}$\footnote{Notice that, by construction, the number of elements in $S_E$ is always $M_E$}.
			...
			(A -A [:,S_E] (B[:,S_E]^{-1} B)) R^{'} = A  Q_L\label{affine_relation_2}
			\end{align}
