...
\begin{document}
	...
	\begin{itemize}
		...
		\end{itemize}
  	\item Frequently, we will work with affine maps on both continuous and discrte spaces.  For example, a $C$ affine map on the disrete space $\R^{}\bar{K}$ can be decomposed  into a $C_L \in \R^{\bar{K}\times K}$ and $C_b \in \R^{\bar{K}}$ such that for some $x \in \R^{\bar{K}}$ with $C : \R^{\bar{K}} \to \R^{\bar{K}}$ and $C x = C_L x + C_b$
	...
	\begin{itemize}
		...
		\item The baseline domain of the operator is $x \in [x^{\min}, x^{\max}]$
		...
		\item Hence, denote grid on the closure as $\bar{x} \in \R^{\bar{K}}$.  The interior points on the grid match the grid on the closure, so that $\bar{x}_{k+K^{\min}_B} = x_{k}$ for $k = 1, \ldots K$ for the $K^{\min}_B$ is the number of points in the boundary to the left of the interior, $K^{\max}_B$ is the number of points to the right, and $K_B = K^{\min}_B + K^{\max}_B$
...
Notice that $B$ is not necessarily unique. The choice of $B$ is exactly the choice of boundary value discretization. For instance, choosing to do first or second order Neumann border conditions is simply the choice of the operator $B$.}
	...
	\begin{itemize}
		...
		\begin{enumerate}
			...
			\end{equation}
			and, therefore, $u[1] = u[2] - dx b[1]$. You can see that this implies that $Q_b = [dx b[1];\mathbf{0}_{K \times 1};dx b[\bar{K}]]$ and that $Q_L$ is the identity matrix for the interior points and $[1,0, 0,...,0]$ for the first row\footnote{So that $Q_L u = u[1] = \bar{u}[2]$.} and $[0,...,0,0,1]$ for the last row.%\footnote{so that $Q_L v = v[1] = u[2]$ and $Q_L v = v[I] = u[I-1]$, respectively.}.
...
1& 0&\dots&0&0\\
 & & \mathbf{I}_K & & \\
0&0&\dots&0&1
\end{bmatrix}%_{\bar{K}\times K}
\quad , \text{ } Q_b = \begin{matrix}
\mathbf{0}_{\bar{K}}
\end{matrix}%_{\bar{K}\times 1}
\end{equation}
